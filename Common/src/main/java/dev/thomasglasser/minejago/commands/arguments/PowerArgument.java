...
public class PowerArgument implements ArgumentType<Power> {
    ...
    public Power parse(StringReader reader) throws CommandSyntaxException {
        ...
        Power power = MinejagoRegistries.POWER.get().get(id);
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        ...
        for (Power power : MinejagoRegistries.POWER.get().stream().toList())
        {
            list.add(MinejagoRegistries.POWER.get().getKey(power));
        }

        SharedSuggestionProvider.suggestResource(list, builder);
