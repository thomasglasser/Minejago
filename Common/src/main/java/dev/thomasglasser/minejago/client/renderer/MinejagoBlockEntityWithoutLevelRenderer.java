    ...
    public void renderByItem(ItemStack pStack, ItemTransforms.TransformType pTransformType, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
        ...
        else if (pStack.is(MinejagoItems.IRON_SPEAR.get()))
        {
            if (pTransformType == ItemTransforms.TransformType.FIXED || pTransformType == ItemTransforms.TransformType.GROUND || pTransformType == ItemTransforms.TransformType.GUI)
            {
                ...
                Minecraft.getInstance().getItemRenderer().render(pStack, pTransformType, false, pPoseStack, pBuffer, pPackedLight, pPackedOverlay, Minecraft.getInstance().getModelManager().getModel(new ModelResourceLocation(Minejago.MOD_ID, "iron_spear", "inventory")));
            }
            else
            {
                pPoseStack.pushPose();
        ...
        else if (pStack.is(MinejagoItems.SCYTHE_OF_QUAKES.get()))
        {
            ...
            {
                pPoseStack.translate(0.5D, 0.5D, 0.5D);
                Minecraft.getInstance().getItemRenderer().render(pStack, pTransformType, false, pPoseStack, pBuffer, pPackedLight, pPackedOverlay, Minecraft.getInstance().getModelManager().getModel(new ModelResourceLocation(Minejago.MOD_ID, "scythe_of_quakes", "inventory")));
            }
            else
            {
                pPoseStack.pushPose();
                pPoseStack.scale(1.0F, -1.0F, -1.0F);
                VertexConsumer vertexconsumer1 = ItemRenderer.getFoilBufferDirect(pBuffer, this.scytheModel.renderType(Minejago.modLoc("textures/entity/scythe_of_quakes.png")), false, pStack.hasFoil());
        ...
        else if (pStack.is(MinejagoItems.IRON_SCYTHE.get()))
        {
            ...
            {
                pPoseStack.translate(0.5D, 0.5D, 0.5D);
                Minecraft.getInstance().getItemRenderer().render(pStack, pTransformType, false, pPoseStack, pBuffer, pPackedLight, pPackedOverlay, Minecraft.getInstance().getModelManager().getModel(new ModelResourceLocation(Minejago.MOD_ID, "iron_scythe", "inventory")));
            }
            else
            {
                pPoseStack.pushPose();
                pPoseStack.scale(1.0F, -1.0F, -1.0F);
                VertexConsumer vertexconsumer1 = ItemRenderer.getFoilBufferDirect(pBuffer, this.scytheModel.renderType(Minejago.modLoc("textures/entity/iron_scythe.png")), false, pStack.hasFoil());
                this.scytheModel.renderToBuffer(pPoseStack, vertexconsumer1, pPackedLight, pPackedOverlay, 1.0F, 1.0F, 1.0F, 1.0F);
                pPoseStack.popPose();
            }
        }
