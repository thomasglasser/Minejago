...
public class MinejagoArmor
{
    ...
    public static class PoweredArmorSet {
        ...
        private final ResourceLocation POWER_ID;

        public PoweredArmorSet(ResourceLocation powerId, String setName, ArmorMaterial material, Class<? extends PoweredArmorItem> armorClass) {
            SET_NAME = setName;
            POWER_ID = powerId;

            HELMET = ARMOR.register(powerId.getPath() + "_" + setName + "_hood", () -> {
                try {
                    return armorClass.getDeclaredConstructor(ResourceLocation.class, ArmorMaterial.class, EquipmentSlot.class, Item.Properties.class).newInstance(powerId, material, EquipmentSlot.HEAD, DEFAULT_PROPERTIES);
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
            });
            CHESTPLATE = ARMOR.register(powerId.getPath() + "_" + setName + "_jacket", () -> {
                try {
                    return armorClass.getDeclaredConstructor(ResourceLocation.class, ArmorMaterial.class, EquipmentSlot.class, Item.Properties.class).newInstance(powerId, material, EquipmentSlot.CHEST, DEFAULT_PROPERTIES);
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
            });
            LEGGINGS = ARMOR.register(powerId.getPath() + "_" + setName + "_pants", () -> {
                try {
                    return armorClass.getDeclaredConstructor(ResourceLocation.class, ArmorMaterial.class, EquipmentSlot.class, Item.Properties.class).newInstance(powerId, material, EquipmentSlot.LEGS, DEFAULT_PROPERTIES);
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
            });
            BOOTS = ARMOR.register(powerId.getPath() + "_" + setName + "_boots", () -> {
                try {
                    return armorClass.getDeclaredConstructor(ResourceLocation.class, ArmorMaterial.class, EquipmentSlot.class, Item.Properties.class).newInstance(powerId, material, EquipmentSlot.FEET, DEFAULT_PROPERTIES);
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
            });
        ...
        public static PoweredArmorSet create(ResourceLocation powerName, String setName, ArmorMaterial material, Class<? extends PoweredArmorItem> clazz)
        {
        ...
        public String getName()
        {
            return POWER_ID.getPath() + "_" + SET_NAME + "_gi";
        }
        ...
        public static PoweredArmorSet getSetForPower(ResourceKey<Power> power, List<PoweredArmorSet> sets) {
            for (PoweredArmorSet set : sets) {
                if (set.POWER_ID.equals(power.location()))
                    return set;
            }
