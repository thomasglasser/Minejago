...
{
	private static final List<EntityFocusModifier> ENTITY_FOCUS_MODIFIERS = new ArrayList<>();
	...
	public static void load(ResourceManager resourceManager) {
		ENTITY_FOCUS_MODIFIERS.clear();
		resourceManager.listResources(Minejago.MOD_ID + "/focus_modifiers/entity", (path) -> path.getPath().endsWith(".json"))
	...
	private static void load(ResourceLocation resourceId, Resource resource) {
		...
		try {
			...
			try {
				...
				EntityFocusModifier.fromJson(id, json).ifPresent(ENTITY_FOCUS_MODIFIERS::add);
	...
	public static double applyModifier(Entity entity, double oldValue) {
		List<EntityFocusModifier> data = ENTITY_FOCUS_MODIFIERS.stream().filter(modifier -> modifier.getEntityType().equals(entity.getType()) && NbtUtils.compareNbt(modifier.getNbt(), new EntityDataAccessor(entity).getData(), true)).toList();
		if (!data.isEmpty())
		{
