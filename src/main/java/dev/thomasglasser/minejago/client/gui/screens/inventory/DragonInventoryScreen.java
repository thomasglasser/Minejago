...
public class DragonInventoryScreen extends AbstractContainerScreen<DragonInventoryMenu> {
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
        ...
        guiGraphics.blit(HorseInventoryScreen.HORSE_INVENTORY_LOCATION, i, j, 0, 0, this.imageWidth, this.imageHeight);
        if (dragon.hasChest()) {
            guiGraphics.blitSprite(HorseInventoryScreen.CHEST_SLOTS_SPRITE, 90, 54, 0, 0, i + 79, j + 17, this.inventoryColumns * 18, 54);
        } else {
            for (int k = 0; k < 4; k++) {
                guiGraphics.blitSprite(HorseInventoryScreen.CHEST_SLOTS_SPRITE, 90, 54, 0, 0, i + 79, j + 17, 18 + k * 18, 18);
            }
        }

        if (this.dragon.isSaddleable()) {
            guiGraphics.blitSprite(HorseInventoryScreen.SADDLE_SLOT_SPRITE, i + 7, j + 35 - 18, 18, 18);
        }
